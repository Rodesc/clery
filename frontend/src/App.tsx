...
const App = () => {
	...
	function deleteFlashMessage (index:number) {
		setFlashMessages(flashMessages?.filter(e => flashMessages.indexOf(e) !== index))
	}
	...
	return (
		<div>
			...
			<FlashMessages messages={flashMessages} deleteFlashMessage={deleteFlashMessage}/>
			<Switch>
				<Route path="/register" >
					{loggedIn ?  <Redirect to="/upload" /> : <Auth showLogin={false}/>}
				</Route>

				<Route path="/login"  >
					{loggedIn ?  <Redirect to="/upload" /> : <Auth showLogin={true} logInUser={logInUser}/>}
				</Route>
